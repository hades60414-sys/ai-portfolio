...
|---|---|---|---|
| Edge Validator | 量化驗證產品 | **public** `hades60414-sys/edge-validator`，遠端 `master` = `7e68e79` | 本機仍有 3 筆既有變更，未推送；先重跑靜態與互動測試再決定 checkpoint |
...
| Task Gacha | 離線任務 PWA | 未建遠端；本機 `c814852`、工作樹乾淨 | **暫停公開**：`npm run lint` 為 17 errors／2 warnings，且 package scripts 無 test；PWA 與外部 Google Forms 回饋連結需完成驗證／揭露 |

## 已驗證的 GitHub 備份

- **公開**：`ai-portfolio`（`main` 與遠端同步，最新精確值以 Git／GitHub Actions 為準）、Edge Validator `7e68e79`、Research Radar `9070e31`。
- **私人**：ChatStock `feature/strategy-stack` `650a5eb`、marketvault `0fef47c`、投資組合儀表板 `143fd20`、選擇權助手 `210a080`、Lab LLM Chat `408aba7`。
...
3. Edge Validator、`daily-flow`、`wild_alpha`、`auto-quant-btc` 的使用者變更保持原樣，沒有為了「看起來乾淨」而自動 commit／push。
4. 族群雷達與 TCRI 即使工作樹乾淨，也分別受資料來源與書面授權邊界阻擋；乾淨不等於可上傳。
5. 新公開 repository、private→public、Pages／部署、憑證輪替、資料庫搬移與真錢功能都不是自動備份步驟。
