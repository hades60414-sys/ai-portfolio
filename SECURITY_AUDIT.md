...
- 正式首頁文件由 Sites worker 回應並加入 CSP（含 `frame-ancestors 'none'`）、`X-Frame-Options: DENY`、Permissions-Policy、COOP/CORP、Referrer-Policy 與 `nosniff`；client asset tree 刻意不保留可繞過 worker 的 `index.html`。頁面本身不載入追蹤器、遠端字型或遠端 JavaScript。
