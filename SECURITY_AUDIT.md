...
- `npm test` 11/11 通過（含非 loopback 綁定拒絕、履歷隱私、公開連結邊界與媒體簽章）；`npm run check` 確認 13 個專案、32 個 source files，未發現明顯秘密或不安全 IP literal。
- 對外專案連結只指向已由 GitHub API 確認為 public 的 repository；ChatStock 實際為 private，因此作品集僅展示案例、不輸出會讓訪客撞 404 的私人外鏈。
- 正式 Sites 靜態層透過 `_headers` 加入 CSP（含 `frame-ancestors 'none'`）、`X-Frame-Options: DENY`、Permissions-Policy、COOP/CORP、Referrer-Policy 與 `nosniff`；worker 也保留同一份 fail-safe policy。頁面本身不載入追蹤器、遠端字型或遠端 JavaScript。
